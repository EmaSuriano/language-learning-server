...
from agents.evaluator import ChatMessage, get_overview
...
BAD_CONVERSATION = [
    ...
    ChatMessage(role="human", content="No se"),
    ChatMessage(
        role="ai",
...
GOOD_CONVERSATION = [
    ...
    ChatMessage(role="human", content="si, hay acaso algún tipo de gimnasio aquì? "),
    ChatMessage(
        role="ai",
        content="Sí, tenemos un pequeño gimnasio en el segundo piso. Está abierto las 24 horas. ¿Le gustaría verlo ahora o más tarde?",
    ),
    ...
    ChatMessage(role="human", content="no lo puedo ver en mi celular sin problemas"),
    ChatMessage(
        role="ai",
        content="Entiendo. Puede encontrar el mapa en la recepción cuando quiera. Si tiene alguna pregunta, llámenos al 123-4567. ¿Hay algo más que pueda hacer por usted hoy?",
    ),
...
async def main():
    ...
    result = await get_overview(user, situation, GOOD_CONVERSATION, rag_evaluator)
